            ...
            switch ($element)
            {
                ...
                case(preg_match("/^(part.\d+)$/", $element) ? true : false):
                    ...
                    if (empty($sibling_id))
                    {
                        ...
                        $part->saveIntoDb($part->position, $this->string_id, $this->caption, $this->textcaption, $parentid);
                        $sibling_id = $part->compid;
                    }
                    else
                    {
                        $part = $this->parts[$partString[1]];
                        $part->saveIntoDb($part->position, $this->string_id, $this->caption, $this->textcaption, $parentid, $sibling_id);
                        $sibling_id = $part->compid;
                    }
                    break;
